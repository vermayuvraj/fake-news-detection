...
% ---------------------------------------------------------------------------
% Author-input marker. Every occurrence flags information that must come from
% the author rather than be inferred. Remove the macro body before submission.
% ---------------------------------------------------------------------------
\newcommand{\authorinput}[1]{%
  \textcolor{red}{\textbf{[Author Input Required:} #1\textbf{]}}}
...
Email: \authorinput{preferred contact e-mail for the camera-ready version}\\
Code and data: \url{https://github.com/vermayuvraj/fake-news-detection}}
\thanks{\authorinput{ORCID identifier, and any acknowledgement of
computing resources, advisors, or funding, if applicable.}}}
...
lossless. We conclude that within-corpus scores on this benchmark quantify
source and topic separability rather than veracity assessment, and we recommend
metadata-only, small-sample, and topic-disjoint baselines as inexpensive
diagnostics that any future study on this corpus can report.
\end{abstract}
...
\cite{khan2021benchmark}.\footnote{\authorinput{if a quantitative statement of
usage is desired (e.g.\ ``used in $N$ published studies''), supply a verifiable
count or citation; we deliberately avoid an unsupported figure here.}} It pairs
...
Core i7-14700HX (28 logical cores), 15.7\,GB RAM, Windows~10 (AMD64). No GPU is
used. The stack is Python~3.10.11 with scikit-learn~1.7.2
\cite{pedregosa2011scikit}, NumPy~1.24.3 \cite{harris2020array},
SciPy~1.15.3 \cite{virtanen2020scipy}, pandas~2.3.3 \cite{mckinney2010data},
and Matplotlib~3.10.8 \cite{hunter2007matplotlib}. Exact versions are pinned in
the released \texttt{requirements.txt}.
...
released pipeline produced bit-identical metrics. Following community
...
inexpensive: the entire study, comprising 20 fitted conditions, runs in roughly
twelve minutes, so the diagnostics we recommend impose no practical barrier.
Second, on cost grounds this pipeline would be an attractive deployment
candidate---which is precisely why its failure under topic shift matters.
...
\begin{table}[t]
\centering
\caption{Measured computational cost (single CPU, no GPU; $N=27{,}177$
training documents, $V=50{,}000$).}
\label{tab:cost}
\footnotesize
\begin{tabular}{@{}lr@{}}
\toprule
...
Our results suggest a reading of the literature's near-ceiling clustering
across model families \cite{khan2021benchmark,kaliyar2021fakebert}. If the
label is recoverable from a redundant, low-complexity provenance signature, then
any sufficiently expressive learner will find it, and additional capacity buys
nothing measurable in-distribution. Worse, capacity cannot help
out-of-distribution either, because the limitation is an identifiability
problem in the data rather than a modelling deficiency: $\mathcal{P}_{\text{bench}}$
does not contain the counterfactual pairs---same publisher, different
veracity---that would be required to separate $c$ from $s$. This is the
mechanism Schuster et al. \cite{schuster2020limitations} describe for
machine-generated text, and our metadata-only and deletion probes are the
analogue of hypothesis-only diagnostics in NLI
\cite{gururangan2018annotation,poliak2018hypothesis}.
...
\textbf{Instrument capacity.} The probe is a bag-of-words linear model. It
cannot represent word order beyond bigrams, negation, or discourse structure.
Our claim is therefore asymmetric and we are careful about its direction: we
show the benchmark \emph{can} be solved by provenance cues and that our probe
does so, not that no model could learn veracity-bearing features. A transformer
might exploit deeper cues; that it would need to is not evident, since it
attains similar in-distribution scores \cite{kaliyar2021fakebert}. Testing
whether a fine-tuned transformer degrades similarly under our topic-disjoint
protocol would materially strengthen the argument, and we have not run it.
...
\textbf{Transformer comparison under identical protocols.} Fine-tuning
DistilBERT \cite{sanh2019distilbert,devlin2019bert} and evaluating it under the
same three protocols would test whether pretrained contextual representations
resist provenance shortcuts or merely encode them more efficiently.
...
correspondingly specific: one corpus, one instrument family, and a shift
protocol that perturbs several provenance factors together
(Section~\ref{sec:limitations}). Within those limits the practical implication
is concrete. Reported accuracy on this benchmark should be read as a measurement
...
\begin{itemize}
  ...
  \item \textbf{Randomness.} Single seed $\sigma=42$ for splits and models;
    ...
    produced bit-identical metrics.
  ...
  \item \textbf{Artifacts.} Machine-readable results
    (\texttt{results.json}, \texttt{shift\_analysis.json}) accompany the code, and
    all reported values are drawn from them.
  \item \textbf{Compute.} Complete study $\approx12$ minutes on one CPU.
\end{itemize}

%=============================================================================
\section*{Acknowledgment}
\authorinput{acknowledgements, if any (advisors, reviewers, institutions).
